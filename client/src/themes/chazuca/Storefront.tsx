...
export default function ChazucaStorefront({
  mode = "demo",
  storeId,
  storeSlug,
  headerVariant,
}: ChazucaStorefrontProps) {
  ...
  const demoProducts: ProductData[] =
    chazucaDemoProducts.map((product) => ({
      id: product.id,
      name: product.name,
      slug: product.slug,
  ...
  const chazucaProducts =
    useMemo<ChazucaProduct[]>(() => {
      const active =
        isDemo
          ? demoProducts
          : products.filter(
