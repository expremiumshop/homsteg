    ...
    return (
      <main className="flex min-h-screen min-w-0 items-center justify-center overflow-x-hidden bg-[#f7f7f5] p-6">
        <div className="w-full min-w-0 max-w-md text-center">
          <div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-2xl bg-white shadow-sm">
    ...
    <main className="min-h-screen min-w-0 overflow-x-hidden bg-[#f7f7f5] px-4 py-8 pb-24 md:px-6 md:py-12">
      <div className="mx-auto w-full min-w-0 max-w-6xl">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
          ...
          <div className="min-w-0 space-y-4">
            {cart.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="flex min-w-0 flex-col gap-4 rounded-xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:gap-5"
                >
                  {/* IMAGEM */}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 min-w-0 break-words text-sm font-black text-red-600">
                      {formatPrice(
                        Number(
                          item.price,
                        ),
                    ...
                    {/* QUANTIDADE */}
                    <div className="mt-4 flex w-fit items-center rounded-lg border border-slate-200">
                      <button
                        type="button"
                        onClick={() =>
                          updateQuantity(
          ...
          {/* RESUMO */}
          <aside className="h-fit min-w-0 rounded-xl border border-slate-200 bg-white p-6 lg:sticky lg:top-24">
            ...
            <Link
              href={
                storeSlug
                  ? `/themes/luxe/checkout?storeSlug=${encodeURIComponent(storeSlug)}`
                  : "/themes/luxe/checkout"
              }
              className="mt-6 flex min-w-0 items-center justify-center rounded-xl bg-red-500 p-4 text-sm font-black text-white transition hover:bg-red-600"
            >
              Finalizar compra
            </Link>
          </aside>
