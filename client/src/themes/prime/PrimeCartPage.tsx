...
export default function PrimeCartPage() {
  const {
    cart,
    removeFromCart,
    updateQuantity,
    ...
    <main className="min-h-screen min-w-0 overflow-x-hidden bg-[#f7f7f8] px-4 py-8 pb-24 md:px-6 md:py-12">
      <div className="mx-auto w-full min-w-0 max-w-6xl">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
          ...
          <div className="min-w-0 space-y-4">
            {cart.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="flex min-w-0 flex-col gap-4 rounded-2xl bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:gap-5"
                >
                  {/* IMAGEM */}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 min-w-0 break-words text-sm font-black text-slate-950">
                      {formatPrice(
                        Number(
                          item.price,
                        ),
                    ...
                    {/* QUANTIDADE */}
                    <div className="mt-4 flex w-fit items-center rounded-lg bg-slate-100 shadow-sm">
                      <button
                        type="button"
                        onClick={() =>
                          updateQuantity(
          ...
          {/* RESUMO */}
          <aside className="h-fit min-w-0 rounded-2xl bg-white p-6 shadow-sm lg:sticky lg:top-24">
            ...
            <Link
              href={
                storeSlug
                  ? `/themes/prime/checkout?storeSlug=${encodeURIComponent(storeSlug)}`
                  : "/themes/prime/checkout"
              }
              className="mt-6 flex min-w-0 items-center justify-center gap-2 rounded-2xl bg-orange-500 p-4 text-sm font-black text-white transition hover:bg-orange-600"
            >
              Finalizar compra
